...
public:
    ...
    gather_type gather(icontext_type& context, const vertex_type& vertex,
        ...
        std::cout << vertex.id() << vertex.data().inside << other.id()
                << other.data().inside << " alpha_beta "
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        std::cout << "id:" << vertex.id() << " " << total.alpha_beta
                << std::endl;
...
struct alpha_beta_type {
    ...
    static alpha_beta_type aggregator(icontext_type& context,
...
struct find_a_b_pair {
    ...
    find_a_b_pair(saedb::vertex_id_type vid, bool inside, int alpha_beta) {
        ...
        if (inside && alpha_beta == global_alpha_beta->beta)
            ...
            a = vid;
        std::cout << "id: " << vid << "a: " << this->a << " b: " << this->b
                << " invalid: " << saedb::INVALID_ID << std::endl;
    }
    ...
    static find_a_b_pair aggregator(icontext_type& context,
            graph_type::vertex_type& vertex) {
...
struct find_a_b_pair_non_neighbor {
    ...
    find_a_b_pair_non_neighbor(saedb::vertex_id_type vid, bool inside,
            int alpha_beta, const set<saedb::vertex_id_type>& neighbor_set) {
        a = -1;
        b = -1;
        finished = false;
    ...
    find_a_b_pair_non_neighbor& operator+=(
            const find_a_b_pair_non_neighbor& other) {
        if (!other.finished && !finished) {
            if (a != saedb::INVALID_ID) {
    ...
    static find_a_b_pair_non_neighbor aggregator(icontext_type& context,
        ...
        return find_a_b_pair_non_neighbor(vertex.id(), vertex.data().inside,
                vertex.data().alpha_beta, neighbor_set);
    }
};
...
struct find_a {
    ...
    static find_a aggregator(icontext_type& context,
            graph_type::vertex_type& vertex) {
...
struct find_b {
    ...
    static find_b aggregator(icontext_type& context,
            graph_type::vertex_type& vertex) {
        if (vertex.data().inside
...
struct find_a_set {
    ...
    static find_a_set aggregator(icontext_type& context,
            graph_type::vertex_type& vertex) {
        if (!vertex.data().inside
                && vertex.data().alpha_beta == global_alpha_beta->alpha)
            vertex.data().inside = true;
        return find_a_set(vertex.id(), vertex.data().inside,
                vertex.data().alpha_beta);
    }
};

void alpha_beta_community_detection(saedb::IEngine<AlphaBeta>* engine,
        graph_type& graph) {
    ...
    auto alpha_beta_result = engine->map_reduce_vertices<alpha_beta_type>(
            alpha_beta_type::aggregator);
    global_alpha_beta = &alpha_beta_result;
    cout << "alpha:" << alpha_beta_result.alpha << " " << "beta:"
            << alpha_beta_result.beta << endl;
    while (global_alpha_beta->alpha >= global_alpha_beta->beta) {
        //swapping
        while (global_alpha_beta->alpha > global_alpha_beta->beta) {
            //perform swapping
            cout << "find_a_b_pair" << endl;
            auto find_a_b_pair_result = engine->map_reduce_vertices<
                    find_a_b_pair>(find_a_b_pair::aggregator);
            ...
            engine->start();
            alpha_beta_result = engine->map_reduce_vertices<alpha_beta_type>(
                    alpha_beta_type::aggregator);
            global_alpha_beta = &alpha_beta_result;
        }
        cout << "find_a_b_pair_non_neighbor" << endl;
        find_a_b_pair_non_neighbor find_a_b_pair_non_neighbor_result =
                engine->map_reduce_vertices<find_a_b_pair_non_neighbor>(
                        find_a_b_pair_non_neighbor::aggregator);
        if (find_a_b_pair_non_neighbor_result.finished) {
            cout << "a: " << find_a_b_pair_non_neighbor_result.a << " b: "
                    << find_a_b_pair_non_neighbor_result.b << endl;
            ...
            cout << "find_a";
            find_a find_a_result = engine->map_reduce_vertices<find_a>(
                    find_a::aggregator);
            if (find_a_result.finished) {
                ...
                cout << "find_b";
                find_b find_b_result = engine->map_reduce_vertices<find_b>(
                        find_b::aggregator);
                if (find_b_result.finished) {
                    ...
                    for (auto ei = vertex_b.in_edges(); ei->Alive(); ei->Next())
                        signal_vertices.push_back(ei->SourceId());
                    for (auto ei = vertex_b.out_edges(); ei->Alive();
                            ei->Next())
                        signal_vertices.push_back(ei->TargetId());
                    //caculate global alpha beta
                    engine->signalVertices(signal_vertices);
                    ...
                    cout << "find_a_set";
                    find_a_set find_a_set_result = engine->map_reduce_vertices<
                            find_a_set>(find_a_set::aggregator);
                }
            }
        }
        break;
...
int main() {
    ...
    for (auto i = 0; i < graph.num_local_vertices(); i++) {
        std::cout << "v[" << i << "]: " << graph.vertex(i).data().inside
                << endl;
    }

    saedb::IEngine<AlphaBeta> *engine = new saedb::EngineDelegate<AlphaBeta>(
            graph);
